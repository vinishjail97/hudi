...
import static org.apache.hudi.common.model.HoodieRecord.HOODIE_IS_DELETED;
...
public class HoodieAdaptablePayloadDataGenerator {
  ...
  public static List<HoodieRecord> getUpdates(List<HoodieRecord> baseRecords, long ts, Class<?> payloadClass) throws IOException {
    ...
    for (HoodieRecord r : baseRecords) {
      GenericRecord gr = (GenericRecord) ((HoodieAvroIndexedRecord)toIndexedRecord(r, SCHEMA, props).get()).getData();
  ...
  private static HoodieRecord getHoodieRecord(GenericRecord r, Class<?> payloadClass) throws IOException {
    ...
    HoodieAvroIndexedRecord indexedRecord =  new HoodieAvroIndexedRecord(r)
        .prependMetaFields(
            SCHEMA,
            SCHEMA_WITH_METAFIELDS,
            new MetadataValues().setRecordKey(r.get("id").toString()).setPartitionPath(r.get("pt").toString()),
            new Properties());
    return wrapIntoHoodieRecordPayloadWithParams(indexedRecord,
            SCHEMA_WITH_METAFIELDS,
  ...
  public static class RecordGen {
    ...
    GenericRecord populateForDelete(GenericRecord r, long ts) {
      ...
      if (payloadClass == MySqlDebeziumAvroPayload.class) {
        ...
        r.put(HOODIE_IS_DELETED, true);
      }
      return r;
    }
  }

  private static Option<HoodieAvroIndexedRecord> toIndexedRecord(HoodieRecord<HoodieRecordPayload> r, Schema recordSchema, Properties props) throws IOException {
    Option<IndexedRecord> avroData = r.getData().getInsertValue(recordSchema, props);
    if (avroData.isPresent()) {
      HoodieAvroIndexedRecord record =
          new HoodieAvroIndexedRecord(r.getKey(), avroData.get(), r.getOperation());
  ...
  public static HoodieRecord<HoodieRecordPayload> wrapIntoHoodieRecordPayloadWithParams(
      HoodieRecord<IndexedRecord> record,
      Schema recordSchema,
      Properties props,
    ...
    return HoodieAvroUtils.createHoodieRecordFromAvro(record.getData(), payloadClass, preCombineField,
        simpleKeyGenFieldsOpt, withOperation, partitionNameOp, populateMetaFields, schemaWithoutMetaFields);
  }
