  ...
  public void addBaseFile(HoodieBaseFile dataFile) {
    ...
    if (fileSlice.getBaseFile().isPresent()) {
      ...
      Option<HoodieInstant> completedInstant = Option.fromJavaOptional(timeline.getInstants()
          .stream()
          .filter(instant -> instant.requestedTime().equals(dataFile.getCommitTime()) && instant.isCompleted())
          .findFirst());
    ...
    try {
      switch (instant.getAction()) {
        ...
        case REPLACE_COMMIT_ACTION:
          HoodieReplaceCommitMetadata replaceCommitMetadata = timeline.readReplaceCommitMetadata(instant);
