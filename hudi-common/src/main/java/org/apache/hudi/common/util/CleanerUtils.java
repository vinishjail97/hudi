...
public class CleanerUtils {
  ...
  public static Option<HoodieInstant> getEarliestCommitToRetain(
      ...
      Instant latestInstant, int hoursRetained) {
    ...
    if (cleaningPolicy == HoodieCleaningPolicy.KEEP_LATEST_COMMITS
      ...
      ZonedDateTime latestDateTime = ZonedDateTime.ofInstant(latestInstant, ZoneId.systemDefault());
      String earliestTimeToRetain = HoodieActiveTimeline.formatDate(Date.from(latestDateTime.minusHours(hoursRetained).toInstant()));
      earliestCommitToRetain = Option.fromJavaOptional(completedCommitsTimeline.getInstants().filter(i -> HoodieTimeline.compareTimestamps(i.getTimestamp(),
