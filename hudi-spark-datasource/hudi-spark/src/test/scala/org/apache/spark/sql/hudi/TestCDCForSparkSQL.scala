...
import org.apache.hudi.common.table.cdc.HoodieCDCSupplementalLoggingMode.WITH_BEFORE_AFTER
    ...
    withTempDir { tmp =>
      ...
      spark.sql(
        s"""
           ...
           |   'hoodie.table.cdc.supplemental.logging.mode' = '${WITH_BEFORE_AFTER.getValue}',
           |   type = 'cow'
           | )
           | location '$basePath'
      """.stripMargin)
