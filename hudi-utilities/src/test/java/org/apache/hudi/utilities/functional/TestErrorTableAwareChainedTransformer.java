...
public class TestErrorTableAwareChainedTransformer extends SparkClientFunctionalTestHarness {
  ...
  public void testForErrorTableConfig() {
    ...
    properties.setProperty(QUARANTINE_TABLE_ENABLED.key(), "true");
    Dataset<Row> transformed = transformer.apply(jsc(), spark(), original, properties);
    List<Row> rows = transformed.collectAsList();
    assertArrayEquals(new String[]{"foo", QUARANTINE_TABLE_CURRUPT_RECORD_COL_NAME}, transformed.columns());
    assertEquals(2, transformed.filter(new Column(QUARANTINE_TABLE_CURRUPT_RECORD_COL_NAME)
        .isNotNull()).count());
  ...
  private Transformer getErrorEventHandlerTransformer() {
    return (jsc, sparkSession, dataset, properties) -> {
      boolean isErrorTableEnabledInTransformer = properties.getBoolean(QUARANTINE_TABLE_ENABLED.key(), QUARANTINE_TABLE_ENABLED.defaultValue());
      if (isErrorTableEnabledInTransformer) {
        dataset = dataset.withColumn(QUARANTINE_TABLE_CURRUPT_RECORD_COL_NAME, functions.when(new Column(QUARANTINE_TABLE_CURRUPT_RECORD_COL_NAME)
                .isNull(),
            functions.lit("true")
        ).otherwise(new Column(QUARANTINE_TABLE_CURRUPT_RECORD_COL_NAME)));
      }
      return dataset;
    };
  }
