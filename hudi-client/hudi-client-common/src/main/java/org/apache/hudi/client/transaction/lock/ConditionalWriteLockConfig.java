...
public class ConditionalWriteLockConfig extends HoodieConfig {
  ...
  public static final ConfigProperty<Long> LOCK_VALIDITY_TIMEOUT_MS =
          ConfigProperty.key(CONDITIONAL_WRITE_LOCK_PROPERTY_PREFIX + "lock_validity_timeout_ms")
                  ...
                  .markAdvanced()
                  .sinceVersion(SINCE_VERSION_0_15_0)
                  .withDocumentation("For storage based conditional write lock provider, the amount of time each new lock is valid for."
  ...
  public static final ConfigProperty<Long> HEARTBEAT_POLL_MS =
          ConfigProperty.key(CONDITIONAL_WRITE_LOCK_PROPERTY_PREFIX + "heartbeat_poll_ms")
                  .defaultValue(TimeUnit.SECONDS.toMillis(30))
                  .markAdvanced()
                  .sinceVersion(SINCE_VERSION_0_15_0)
                  .withDocumentation("For storage based conditional write lock provider, the amount of time to wait before renewing the lock. Defaults to 30 seconds.");
  ...
  public static class Builder {
    ...
    private void checkRequiredProps() {
      ...
      if (lockConfig.getLongOrDefault(LOCK_VALIDITY_TIMEOUT_MS) < 5000) {
        throw new IllegalArgumentException(LOCK_VALIDITY_TIMEOUT_MS.key() + " should be greater than or equal to 5 seconds.");
      }
      if (lockConfig.getLongOrDefault(HEARTBEAT_POLL_MS) < 1000) {
        throw new IllegalArgumentException(HEARTBEAT_POLL_MS.key() + " should be greater than or equal to 1 second.");
      }
    }
  }
}